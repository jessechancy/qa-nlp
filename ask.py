...
def WH_question_from_tree(parsed_tree):
    ...
    for ent in doc.ents:
        ...
        label = ent.label
        if label =="GPE":
            question.append("Where {npWord} {vpWord} ?")
        elif label == 383: #"PERSON":
            question.append("Who {vpWord}?")
        elif label == 381: # "DATE":
            question.append("When {vpWord} ?")
        elif label == 380: # "object":
            question.append("What {vpWord} ?")
    ...
    if innerNPtype == "CD":
        question.append("How many {npWord} ?")
    if innerNPtype == "NNS":
        #question.append(f"Who are {npWord} ?")
        question.append("What are {npWord} ?")
        question.append("What {npWord} {vpWord} ?")
        question.append("Why do {npWord} {vpWord} ?")
    assert (np.label() == 'NP')
    assert (vp.label() == 'VP')
    '''
    if vp[0].label() == 'VBZ':
